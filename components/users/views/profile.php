...
<script language="javascript">
	jQuery(document).ready(function ($){
		$("#profile_upload_image").change(function () {
			$.ajaxFileUpload({
				url:'<?php echo route("users/profile.image"); ?>',
				secureuri:false,
...
<div class="col-md-9">
	<?php echo Form::create("profile")->action(route("users/profile"))->method("POST"); ?>
      <div class="box box-primary">
        <div class="box-header with-border">
		...
		<?php
			...
			if($user->hasPerm("user.role")) {
				...
				echo Form::_("select2", "Roles", "roles")->options($roles)->select($selected)->multiple();
			}
		?>
