...
function apprise(string, args, callback) {
    var default_args =
		{
		    ...
		    'textOk': Translate.OK, 	// Ok button default text
		    'textCancel': Translate.CANCEL, // Cancel button default text
		    'textYes': Translate.YES, 	// Yes button default text
		    'textNo': Translate.NO, 	// No button default text
    ...
    $('.aButtons > button').click(function () {
        ...
        if (callback) {
            ...
            if (wButton == 'ok') {
                if (args) {
                    if (args['input']) { callback(aText,jQuery(".appriseInner")); }
                    else { callback(true,jQuery(".appriseInner")); }
                }
                else {
					
                    callback(true,jQuery(".appriseInner")); 
                }
            }
            else if (wButton == 'cancel') {
                callback(false,jQuery(".appriseInner")); 
            }
        }
		overlay.remove();
		apprise.remove();
